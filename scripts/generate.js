...
for (const [name, colors] of Object.entries(VARIANTS)) {
	...
	await writeFile(join(__dirname, `../${lowerName}.yazi/flavor.toml`), s.replaceAll("${variant}", lowerName))

	// README.md
	await writeFile(
		join(__dirname, `../${lowerName}.yazi/README.md`),
		readme
			.replaceAll("${mode}", name === "Catppuccin Latte" ? "light" : "dark")
